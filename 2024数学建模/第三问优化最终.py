...
for comb in valid_combinations:
    ...
    e_defect_rates = []  # 存储每个零件的次品率

    for i in range(8):  # 对每个零件进行抽样检测
        p = data[f'p{i+1}']
        detected_defects = np.random.binomial(n=n_sample, p=p)  # 模拟抽样检测次品率
        e_defect_rate = detected_defects / n_sample  # 推测出的次品率
        e_defect_rates.append(e_defect_rate)  # 存入估计的次品率

    # 根据抽样检测结果决定是否进行全面检测
    N1_into_semi_pro = (1 - e_defect_rates[0]) * N1 if comb[0] == 1 else N1
    N2_into_semi_pro = (1 - e_defect_rates[1]) * N2 if comb[1] == 1 else N2
    N3_into_semi_pro = (1 - e_defect_rates[2]) * N3 if comb[2] == 1 else N3
    N4_into_semi_pro = (1 - e_defect_rates[3]) * N4 if comb[3] == 1 else N4
    N5_into_semi_pro = (1 - e_defect_rates[4]) * N5 if comb[4] == 1 else N5
    N6_into_semi_pro = (1 - e_defect_rates[5]) * N6 if comb[5] == 1 else N6
    N7_into_semi_pro = (1 - e_defect_rates[6]) * N7 if comb[6] == 1 else N7
    N8_into_semi_pro = (1 - e_defect_rates[7]) * N8 if comb[7] == 1 else N8
    ...
    p1_semi_pro_bad = sample_semi_pro_defect_rate(
        [e_defect_rates[0], e_defect_rates[1], e_defect_rates[2]], n_sample, comb[8])
    p2_semi_pro_bad = sample_semi_pro_defect_rate(
        [e_defect_rates[3], e_defect_rates[4], e_defect_rates[5]], n_sample, comb[9])
    p3_semi_pro_bad = sample_semi_pro_defect_rate(
        [e_defect_rates[6], e_defect_rates[7]], n_sample, comb[10])
    ...
    # 成品中的总次品率
    p_pro_bad = np.mean([p1_semi_pro_bad, p2_semi_pro_bad, p3_semi_pro_bad])
